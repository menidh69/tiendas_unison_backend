...
router.get("/stripeInfo/accountLink/:id_tienda", async (req, res) => {
  const infoStripe = await Info_Stripe.findOne({
...
router.get("/paySecret/:id_stripe/:id_user", async (req, res) => {
  ...
  // }
  const customer_info = await Stripe_Customer.findOne({
  ...
  const secrets = await Promise.all(
    tiendas.map(async (tienda) => {
      ...
      const infoTienda = await Info_Stripe.findOne({
...
const handleSuccessfulPaymentIntent = async (paymentIntent) => {
  const infoTienda = await Info_Stripe.findOne({
    where: { id_stripe: paymentIntent.on_behalf_of },
    raw: true,
  });
  const infoCliente = await Stripe_Customer.findOne({
    where: { id_stripe: paymentIntent.customer },
    raw: true,
  });
  ...
  const items = await Carrito.findAll({
    ...
    include: {
      ...
      include: {
        model: Productos,
        where: { id_tienda: infoTienda.id_tienda },
      },
    },
    raw: true,
  });
...
router.post("/stripe/create-subscription", async (req, res) => {
  ...
  const registroSub = await Subscripcion_Tienda.create({
...
router.get("/stripe/subscripcion/:id_tienda", async (req, res) => {
  const subscripcion = await Subscripcion_Tienda.findOne({
    where: { id_tienda: req.params.id_tienda },
  });
...
router.get("/stripe/customer/:id_user", async (req, res) => {
  const info = await Stripe_Customer.findOne({
    where: { id_usuario: req.params.id_user },
  });
