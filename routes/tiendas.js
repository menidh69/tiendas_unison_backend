            ...
            .then(async (usuario) => {
              ...
              await Tienda.create(tienda)
                .then(async (tiendacreada) => {
                  ...
                  await Balance.create(balance)
                    ...
                    .catch((err) => {
                      res.status(400).json({
                        mensaje:
                          "Ocurrio un error al crear la tienda y balance",
                        error: err,
                      });
                    });
                })
                .catch((err) => {
                  res.status(400).json({
                    status:
                      "Ocurrio un error al crear la tienda, vuelve a intentarlo",
                    error: err,
                  });
                });
            })
...
router.get("/tiendas", async (req, res) => {
  const todas = await Tienda.findAll({ raw: true }).then((result) => {
    res.json(result);
  });
});
...
router.get("/universidades/tiendas/:id_universidad/all", async (req, res) => {
  const todas = await Usuario.findAll({
    ...
    include: { model: Tienda, where: { activo: 1 } },
    raw: true,
  }).then((result) => {
    console.log(result);
    res.json(result);
  ...
  const todas = await entities.Tienda.findAll(
    {
      ...
      include: [{ model: entities.Review_Tienda }],
    },
    { raw: true }
  ).then((result) => {
...
router.put("/tiendas/:id", async (req, res) => {
  const tienda = await Tienda.update(
    {
...
router.delete("/tiendas/:id", async (req, res) => {
  try {
    const deleteTienda = await Tienda.destroy({
      where: { id_usuario: req.params.id },
    });
...
router.put("/tiendas/ubicacion/:id_tienda", async (req, res) => {
  const ubicacion = Ubicacion.update(
    ...
    .then((result) => {
      if (result.error) {
        ...
        return res.json({
          status: "Ubicacion actualizada con exito",
          result: result,
        });
      }
    })
...
router.get("/tiendas/pedidosPendientes/:id_tienda", async (req, res) => {
  const ordenes = await sequelize.query(
    "SELECT t1.id, t1.fecha, t3.nombre AS nombre_producto, t2.cantidad, t3.precio, t3.url_imagen, t4.nombre, t4.apellidos FROM orden t1" +
